...
- For public repositories, keep `d1_database_id`/`kv_namespace_id` placeholders in `wrangler.toml` and deploy with `npm --prefix mail-worker run deploy` so IDs are injected from environment variables.
