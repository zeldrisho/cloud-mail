...
const dbInit = {
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'This project is for learning and communication only. Do not use it for illegal activities\n' +
			'<br>\n' +
			'Please follow local laws. The author assumes no legal liability'
	...
	async v1_5DB(c) {
		...
		try {
			...
			console.warn(`Skip adding field: ${e.message}`);
		}
	},
    ...
    `).run();
		...
		try {
			...
			console.warn(`Skip creating index: ${e.message}`);
		}
        ...
        (33,'Registration Key', NULL, 0, 1, 5.1),
        (34,'Key View', 'reg-key:query', 33, 2, 0),
        (35,'Key Add', 'reg-key:add', 33, 2, 1),
        (36,'Key Delete', 'reg-key:delete', 33, 2, 2)`).run();
		} catch (e) {
	...
	async v1_2DB(c){
        ...
        (32,'Data View', 'analysis:query', 31, 2, 1)`).run();
		} catch (e) {
			console.warn(`Skip data: ${e.message}`);
		}
    ...
    `).run();
        ...
        INSERT INTO perm (perm_id, name, perm_key, pid, type, sort) VALUES
        (1, 'Email', NULL, 0, 0, 0),
        (2, 'Email Delete', 'email:delete', 1, 2, 1),
        (3, 'Email Send', 'email:send', 1, 2, 0),
        (4, 'Personal Settings', '', 0, 1, 2),
        (5, 'Delete User', 'my:delete', 4, 2, 0),
        (6, 'User Info', NULL, 0, 1, 3),
        (7, 'User View', 'user:query', 6, 2, 0),
        (8, 'Password Change', 'user:set-pwd', 6, 2, 2),
        (9, 'Status Change', 'user:set-status', 6, 2, 3),
        (10, 'Role Change', 'user:set-type', 6, 2, 4),
        (11, 'User Delete', 'user:delete', 6, 2, 7),
        (12, 'User Favorite', 'user:star', 6, 2, 5),
        (13, 'Role Control', '', 0, 1, 5),
        (14, 'Role View', 'role:query', 13, 2, 0),
        (15, 'Role Edit', 'role:set', 13, 2, 1),
        (16, 'Role Delete', 'role:delete', 13, 2, 2),
        (17, 'System Settings', '', 0, 1, 6),
        (18, 'Settings View', 'setting:query', 17, 2, 0),
        (19, 'Settings Edit', 'setting:set', 17, 2, 1),
        (21, 'Mailbox Sidebar', '', 0, 0, 1),
        (22, 'Mailbox View', 'account:query', 21, 2, 0),
        (23, 'Mailbox Add', 'account:add', 21, 2, 1),
        (24, 'Mailbox Delete', 'account:delete', 21, 2, 2),
        (25, 'User Add', 'user:add', 6, 2, 1),
        (26, 'Reset Send Count', 'user:reset-send', 6, 2, 6),
        (27, 'Email List', '', 0, 1, 4),
        (28, 'Email View', 'all-email:query', 27, 2, 0),
        (29, 'Email Delete', 'all-email:delete', 27, 2, 0),
				(30, 'Role Add', 'role:add', 13, 2, -1)
      `).run();
		}
